...
// Fills an array with the sum of each row from the matrix
void FillSumOfEachRowInArray(int Matrix[][3], int arr[], int rows, int cols) {
    for (int i = 0; i < rows; i++) {
        arr[i] = RowSum(Matrix, i, cols);
    }
}

// Prints the pre-calculated sum of each row from the array
void PrintRowSumInArray(int arr[], int rows) {
    for (int i = 0; i < rows; i++) {
...
int main()
{
    ...
    FillSumOfEachRowInArray(Matrix, arr, 3, 3);

    cout << "The following are the sum of each row in the matrix: " << endl;
    PrintRowSumInArray(arr, 3);
    cout << endl;
